...
class BaseCTPipeline:
    """
    DICOM 폴더 → CT 정보 → feature / label 경로 준비까지 공통 파이프라인.
    (폴더만 지원)
    장기(lung 등) 구분은 하위 클래스에서 한다.
    """
    ...
    def _validate_input_folder(self, folder: Path) -> None:
        ...
        if not folder.exists():
            raise FileNotFoundError(f"입력 폴더를 찾을 수 없습니다: {folder}")
        if not folder.is_dir():
            raise ValueError(
                "입력은 ZIP/파일이 아니라 'DICOM 폴더'여야 합니다. (폴더만 지원)"
            )
    ...
    def _extract_id_from_folder(self, folder: Path) -> str:
        """
        폴더 이름에서 케이스 ID를 만든다.
        예) '/path/to/case001' -> 'case001'
        (폴더만 지원하므로 .zip 처리 로직은 제거)
        """
        return folder.name
    ...
    def load_ct(self, folder: Path, progress_cb: ProgressCB = None) -> Dict[str, Any]:
        ...
        # 1단계
        self._progress(progress_cb, 30, "DICOM 로드 중...")
        dcm = DICOMProcessor()
